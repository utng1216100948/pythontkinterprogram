...
class Database:
    def __init__(self, master, *args, **kwargs):
        ...
        self.btn_add = Button(master, text="Update Database", width=25, height=2, bg='steelblue', 
    ...
    def update(self, *args, **kwargs):
        ...
        query = "UPDATE Inventory SET stock=?, month=? WHERE id=?"
        c.execute(query, (self.u2, self.u9, self.id_leb.get()))
        query1 = "UPDATE product SET name=?, cp=?, sp=?, totalcp=?, totalsp=? WHERE id=?"
        c.execute(query1, (self.u1, self.u3, self.u4, self.u5, self.u6, self.id_leb.get()))
        query2 = "UPDATE vendor SET name=?, phone_num=? WHERE id=?"
        c.execute(query2, (self.u7, self.u8, self.id_leb.get()))
        conn.commit()
